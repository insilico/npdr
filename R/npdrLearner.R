...
#'
#' @examples
#' test.results <- npdrLearner(train.outcome="class", train.data=case.control.data, 
#'                  test.outcome="class", test.data=case.control.3sets$validation,
#'                  nb.method = "relieff", 
#'                  nb.metric = "manhattan", 
#'                  sd.vec = NULL, sd.frac = 0.5, dopar.nn=FALSE, 
#'                  knn=0) # knn=0 uses multisurf k estimate or specify a value
...
npdrLearner <- function(train.outcome, train.data, test.outcome, test.data, 
  ...
  if (length(train.outcome)==1){
    ...
    train.pheno <- train.data[, train.outcome] %>% as.character() %>% as.numeric() # get phenotype
  ...
  if (length(test.outcome)==1){
    # e.g., outcome="class" or outcome=101 (pheno col index) and dataset is data.frame including outcome variable
    test.pheno <- test.data[, test.outcome] %>% as.character() %>% as.numeric() # get phenotype
  ...
  test.predict <- sapply(1:length(test.neighbors),
                         function(i){
                           table(train.pheno[test.neighbors[[i]]]) %>% which.max %>% names() %>% as.numeric()
                         }
  )
